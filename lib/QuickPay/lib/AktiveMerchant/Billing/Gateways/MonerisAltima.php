 ...
 */
class MonerisAltima extends Gateway
{
    ...
    const CREDIT = 'refund';
    const VERIFICATION="card_verification";
 ...
 /**
     ...
     * @return Response
     */
     public function verificarion(CreditCard $creditcard, $options = array())
     {
        $action = static::VERIFICATION;
        if (isset($options['crypt_type'])) {
            $this->crypt_type = $options['crypt_type'];
        }
        if (isset($options['cust_id'])) {
            $this->xml->cust_id($options['cust_id'], $action);
        }
    ...
    private function addCreditcard(CreditCard $creditcard, $action)
    {
        ...
        if($creditcard->verification_value!=null&&trim($creditcard->verification_value)!="")
        {
