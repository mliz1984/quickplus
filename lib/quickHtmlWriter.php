...
class QuickHtmlWriter extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter {
   ...
   public function getHtmlString($encode=null)
   {
		...
		$result = $this->generateHTMLHeader(!$this->_useInlineCss);
		$result .=$this->generateNavigation();
		$result .=$this->generateSheetData();
		$result .=$this->generateHTMLFooter();
		return StringTools::conv($result,$encode);
   }
	/**
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		...
		$html .= '<html>' . PHP_EOL;
		$html .= '  <head>' . PHP_EOL;
		$html .= '	  <meta http-equiv="Content-Type" content="text/html; charset='.$this->encode.'">' . PHP_EOL;
			...
			if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
				if ($drawing->getCoordinates() == $coordinates) {
					...
					$html .= '<img style="position: absolute; z-index: 1; left: ' . 
                        ...
                        UrlTools::getFullUrl($imageData,true) . '" border="0" />';
					$html .= '</div>';
				}
			}
		}
...
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	private function _generateTableHeader($pSheet) {
		...
		if (!$this->_useInlineCss) {
			...
			$html .= '	<table id="'.$this->getTableId().'" border="0" cellpadding="0" cellspacing="0" id="sheet' . $sheetIndex . '" class="sheet' . $sheetIndex . $gridlines . '">' . PHP_EOL;
		} else {
			...
			foreach ($pValues as $cellAddress) {
				...
				if ($writeCell) {
					...
					$html .= '			<' . $cellType;
						...
						if($this->htmlWriterExtend!=null)
						{
							$html.= ' '.$this->htmlWriterExtend->getCellAttr($cellData,PHPExcel_Cell::stringFromColumnIndex($colNum),$pRow + $rowSpan).' ';
						}
					$html .= '>';
					...
					if($this->htmlWriterExtend!=null)
					{
							$cellData = ' '.$this->htmlWriterExtend->getCellData($cellData,PHPExcel_Cell::stringFromColumnIndex($colNum),$pRow + $rowSpan).' ';
					}
					// Cell data
					$html .= $cellData;
					...
					if($this->htmlWriterExtend!=null)
					{
							$html .= ' '.$this->htmlWriterExtend->getAfterCell($cellData,PHPExcel_Cell::stringFromColumnIndex($colNum),$pRow + $rowSpan).' ';
					}
					$html .= PHP_EOL;
				}
