function SortableTable(oTable, oSortTypes, iSkipLastRows) {

	this.sortTypes = oSortTypes || [];
	if (iSkipLastRows == null)
	  this.skipLastRows = 0;
	else
	  this.skipLastRows = iSkipLastRows;
...
SortableTable.prototype.getCache = function (sType, nColumn) {
	...
	var l = rows.length;
	var a = new Array(l - this.skipLastRows);
	var r;
	for (var i = 0; i < l - this.skipLastRows; i++) {
		r = rows[i];
		a[i] = {
...
SortableTable.prototype.getSkipCache = function () {
	if (!this.tBody) return [];
	var rows = this.tBody.rows;
	var l = rows.length;
	var a = new Array(this.skipLastRows);
	var r;
	for (var i = 0; i < this.skipLastRows; i++) {
		r = rows[i + l - this.skipLastRows];
		a[i] = r;
	};
	return a;
};
